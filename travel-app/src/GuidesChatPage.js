...
import './GuidesChatPage.css';
import EmojiPicker from 'emoji-picker-react';


const GuidesChatPage = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        const convRes = await axios.get(`http://127.0.0.1:8000/guidechat/conversations/${userId}`);
        setConversations(convRes.data.conversations);
        
        // Only fetch user details and messages if otherUserId is provided
        if (otherUserId) {
          const userRes = await axios.get(`http://127.0.0.1:8000/guidechat/user-info/${otherUserId}`);
          setOtherUser(userRes.data);
          
          const messagesRes = await axios.get(
            `http://127.0.0.1:8000/guidechat/messages/${userId}/${otherUserId}`
          );
    ...
    const checkForNewMessages = async () => {
      ...
      try {
        const hasNewRes = await axios.get(
          `http://127.0.0.1:8000/guidechat/has-new-messages/${userId}/${otherUserId}/${lastMessageId}`
        );
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await axios.post('http://127.0.0.1:8000/guidechat/messages', messageToSend);
    } catch (err) {
            ...
            <div className="conversation-list">
              {conversations.map((conv) => (
                <Link
                  ...
                  to={`/guidechat/${userId}/${conv.other_user_id}`}
